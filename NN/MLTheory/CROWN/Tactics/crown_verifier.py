#!/usr/bin/env python3
"""Produce CROWN/IBP bounds for TorchLean certificate workflows.

The program computes numerical claims and serializes them as JSON or Lean data. The generated Lean
theorem remains conditional on `LyapunovCert.ValidFor`; serialization alone is not a proof that the
bounds enclose the supplied model.
...
The tool can emit a JSON certificate with V, gradient, and Vdot bounds, or Lean syntax for importing
the data into a checked workflow.
"""
...
def format_lean(cert: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"def certVLower : Float := {V_lo}",
        f"def certVUpper : Float := {V_hi}",
        f"def certDerivativeLower : Float := {Vdot_lo}",
        f"def certDerivativeUpper : Float := {Vdot_hi}",
        f"",
...
def format_lean_full(
    ...
    """Generate Lean certificate data and the theorems justified by its numeric margins."""
    V_lo = cert['V_bounds']['lo']
    V_hi = cert['V_bounds']['hi']
    Vdot_lo = cert['Vdot_bounds']['lo']
    Vdot_hi = cert['Vdot_bounds']['hi']
...
/-!
# Bounds reported by Python CROWN

These declarations record the producer output. They are data, not a proof that the bounds enclose
...
-- Numeric bounds and input region reported by the producer.
noncomputable def cert : RealCert n where
  vLower := {V_lo}
  vUpper := {V_hi}
  derivativeLower := {Vdot_lo}
  derivativeUpper := {Vdot_hi}
  regionLower := fun i => {region_lo_cases}
  regionUpper := fun i => {region_hi_cases}

-- Canonical TorchLean certificate value.
noncomputable def lyapCert : LyapunovCert ℝ n := cert.toCert

/-!
## Semantic boundary

The generated numbers do not prove their own relationship to a network. This module proves only
strict arithmetic facts about those numbers. A checked TorchLean graph workflow must construct
`LyapunovCert.ValidFor` before the bounds can support a stability theorem.
-/

/-!
# Numeric margin proofs
-/
'''
    ...
    if V_positive_ok:
        lean_code += f'''
-- The producer-reported lower bound for `V` is positive.
theorem cert_v_positive : cert.vLower > 0 := by
  simp only [cert]
  norm_num

-- The canonical certificate retains the same positive lower bound.
theorem lyap_cert_v_positive : lyapCert.vLower > 0 := by
  simp only [lyapCert, RealCert.toCert, cert]
  norm_num
'''
    else:
        lean_code += f'''
-- The reported lower bound {V_lo} is not positive, so no positivity theorem is emitted.
-- (We intentionally do not emit unverifiable Lean proofs here.)
'''
    
    if Vdot_negative_ok:
        lean_code += f'''
-- The producer-reported upper bound for the orbital derivative is negative.
theorem cert_vdot_negative : cert.derivativeUpper < 0 := by
  simp only [cert]
  norm_num

-- The canonical certificate retains the same negative derivative upper bound.
theorem lyap_cert_vdot_negative : lyapCert.derivativeUpper < 0 := by
  simp only [lyapCert, RealCert.toCert, cert]
  norm_num
'''
    else:
        lean_code += f'''
-- The reported derivative upper bound {Vdot_hi} is not negative, so no theorem is emitted.
-- (We intentionally do not emit unverifiable Lean proofs here.)
'''
...
'''
    ...
    if not verified:
        lean_code += f'''
/-!
# Producer margin check failed

The producer did not establish both strict numeric margins:
