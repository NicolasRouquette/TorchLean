...
/-- Compile the Lyapunov loss while keeping its large polymorphic program out of callers' code. -/
@[noinline] def compileLoss
    (width : Nat)
    (parameters : _root_.TorchLean.TensorPack Scalar (Core.paramShapes width)) :
    Except String (NN.Verification.TorchLean.CompiledIR Scalar) :=
...
/-- Run and print the IBP bound for an already compiled loss graph. -/
@[noinline] def reportIBP
    (compiled : NN.Verification.TorchLean.CompiledIR Scalar)
...
/-- Run and print the CROWN bound for an already compiled loss graph. -/
...
def checkLossBox
  ...
  let compiled ←
    match compileLoss width parameters with
  ...
  reportIBP compiled parameterStore
  reportCROWN compiled inputBox parameterStore
