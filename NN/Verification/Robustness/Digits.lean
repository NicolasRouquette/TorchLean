...
Run:
  ...
  `lake exe verify -- digits --scalar ieee32-exec --eps=0.02`
...
def usage : String :=
  String.intercalate "\n" [
    ...
    "",
    "Notes:",
    "  - Scalar-semantics flags are also accepted (e.g. --scalar ieee32-exec).",
...
def main (args : List String) : IO Unit := do
  ...
  TorchLean.Runtime.ScalarMode.log scalarMode
  let opts ←
    match parseArgs rest with
    | .ok o => pure o
    | .error msg => throw <| IO.userError msg
  NN.Verification.TorchLean.withBoundScalar scalarMode
    (fun {α} _ _ _ _ _ => do
