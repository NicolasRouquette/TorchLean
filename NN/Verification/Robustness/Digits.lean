...
structure TrainCertifyOpts where
  ...
  script : String := "scripts/verification/robustness/train_digits_linear.py"
...
def parseTrainCertifyArgs : List String → Except String TrainCertifyOpts
  | [] => .ok {}
  | a :: rest =>
    ...
    | .ok o =>
      if a == "--" then
        ...
        .error usage
      else if a.startsWith "--script=" then
        .ok { o with script := (a.drop 9).toString }
      else if a.startsWith "--weights=" then
        .ok { o with certify := { o.certify with weights := (a.drop 10).toString } }
      else if a.startsWith "--dataset=" then
        .ok { o with certify := { o.certify with dataset := (a.drop 10).toString } }
      else if a.startsWith "--eps=" then
        match parseFloatLit (a.drop 6).toString with
        | some v => .ok { o with certify := { o.certify with eps := v } }
        | none => .error s!"bad --eps: {a}"
      else if a.startsWith "--max=" then
        match (a.drop 6).toString.toNat? with
        | some v => .ok { o with certify := { o.certify with max := v } }
        | none => .error s!"bad --max: {a}"
      else if a.startsWith "--seed=" then
        match (a.drop 7).toString.toNat? with
        | some v => .ok { o with seed := v }
        | none => .error s!"bad --seed: {a}"
      else if a.startsWith "--epochs=" then
        match (a.drop 9).toString.toNat? with
        | some v => .ok { o with epochs := v }
        | none => .error s!"bad --epochs: {a}"
      else if a.startsWith "--batch=" then
        match (a.drop 8).toString.toNat? with
        | some v => .ok { o with batch := v }
        | none => .error s!"bad --batch: {a}"
      else if a.startsWith "--lr=" then
        match parseFloatLit (a.drop 5).toString with
        | some v => .ok { o with lr := v }
        | none => .error s!"bad --lr: {a}"
      else if a.startsWith "--test-size=" then
        match parseFloatLit (a.drop 12).toString with
        | some v => .ok { o with testSize := v }
        | none => .error s!"bad --test-size: {a}"
      else if a.startsWith "--max-test=" then
        match (a.drop 11).toString.toNat? with
        | some v => .ok { o with maxTest := v }
        | none => .error s!"bad --max-test: {a}"
      else
        .error s!"unknown arg: {a}\n\n{usage}"
