...
def run : IO Unit := do
  ...
  let embedDim : Nat := 5
  let startPos : Nat := 7
  let sX : Shape := [batch, seqLen, embedDim]
  let sPE : Shape := [seqLen, embedDim]
  ...
  let headDim : Nat := 4
  let startPosR : Nat := 5
  let sR : Shape := [batch, numHeads, seqLen, headDim]
  let sCS : Shape := [seqLen, headDim]

  let cosT : Tensor Float sCS :=
    TorchLean.Tensor.stack 0 fun (pos : Fin seqLen) =>
      Spec.ropeCosVectorSpec (α := Float) (startPosR + pos.val) headDim
  ...
  let signRow : Tensor Float [1, headDim] :=
    TorchLean.Tensor.generate [1, headDim] fun coordinate =>
      let idx := coordinate.getD 1 0
  ...
  let sCS4 : Shape := [1, 1, seqLen, headDim]
  let cbCS4 : Shape.CanBroadcastTo sCS4 sR := (inferInstance : Shape.BroadcastTo sCS4 sR).proof
  let cbSign : Shape.CanBroadcastTo [1, headDim] sFlat :=
    (inferInstance : Shape.BroadcastTo [1, headDim] sFlat).proof

  -- CPU tape
  let t0r : Tape Float := Tape.empty
  let (t1r, xIdr) := Tape.leaf (t := t0r) xR (name := some "x")
  let (t2r, cosIdr) := Tape.leaf (t := t1r) cosT (name := some "cos") (requiresGrad := false)
  ...
  let (t12r, xPermIdr) ← Utils.okOrThrow
    (Tape.indexSelect (α := Float) (s := [headDim, rowsFold])
