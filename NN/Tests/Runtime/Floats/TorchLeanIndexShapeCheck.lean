...
def gradGatherVec (backend : _root_.Runtime.Autograd.Torch.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
...
def gradGatherScalarNat (backend : _root_.Runtime.Autograd.Torch.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← _root_.Runtime.Autograd.TorchLean.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.vector (α := Float) [1.0, 2.0, 3.0]
  let x ← _root_.Runtime.Autograd.TorchLean.Session.input sess xVal (name := some "x") (requiresGrad := true)
...
def gradGatherVecNat (backend : _root_.Runtime.Autograd.Torch.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← _root_.Runtime.Autograd.TorchLean.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.vector (α := Float) [1.0, 2.0, 3.0]
...
def gradGatherRowsNat (backend : _root_.Runtime.Autograd.Torch.Backend) : IO (Tensor Float (.dim 3 (.dim 2 .scalar))) :=
  do
  let sess ← _root_.Runtime.Autograd.TorchLean.Session.new (α := Float) (opts := { backend := backend })
...
def gradBroadcastScalar (backend : _root_.Runtime.Autograd.Torch.Backend) : IO Float := do
  let sess ← _root_.Runtime.Autograd.TorchLean.Session.new (α := Float) (opts := { backend := backend })
...
def gradReshapeMat (backend : _root_.Runtime.Autograd.Torch.Backend) : IO (Tensor Float (.dim 2 (.dim 3 .scalar))) := do
  let sess ← _root_.Runtime.Autograd.TorchLean.Session.new (α := Float) (opts := { backend := backend })
...
def gradTransposeMat (backend : _root_.Runtime.Autograd.Torch.Backend) : IO (Tensor Float (.dim 2 (.dim 3 .scalar))) :=
  do
  let sess ← _root_.Runtime.Autograd.TorchLean.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 2 (.dim 3 .scalar)) :=
...
def gradReduceMeanVec (backend : _root_.Runtime.Autograd.Torch.Backend) : IO (Tensor Float (.dim 3 .scalar)) := do
  let sess ← _root_.Runtime.Autograd.TorchLean.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.vector (α := Float) [1.0, 2.0, 3.0]
  let x ← _root_.Runtime.Autograd.TorchLean.Session.input sess xVal (name := some "x") (requiresGrad := true)
...
def gradScatterAddVec (backend : _root_.Runtime.Autograd.Torch.Backend) : IO (Tensor Float (.dim 3 .scalar) × Float) :=
  do
  let sess ← _root_.Runtime.Autograd.TorchLean.Session.new (α := Float) (opts := { backend := backend })
  let xVal : Tensor Float (.dim 3 .scalar) := NN.Tensor.vector (α := Float) [1.0, 2.0, 3.0]
