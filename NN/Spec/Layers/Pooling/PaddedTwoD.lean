...
def maxPool2dMultiSpecPad {kH kW inH inW inC stride padding : Nat}
  ...
  let _ := layer
  let config := Pooling.Internal.maxPool2dConfig padding h1 h2 hStride
  ...
  exact tensorCast _
    (Pooling.Internal.pool2d_multi_out_shape_eq inC inH inW kH kW stride padding) output
...
def avgPool2dMultiSpecPad {kH kW inH inW inC stride padding : Nat}
    ...
    Tensor α (pool2dMultiOutShapePad inC inH inW kH kW stride padding) := by
  let _ := layer
  let config := Pooling.Internal.avgPool2dConfig padding h1 h2 hStride
...
def maxPool2dMultiBackwardSpecPad {kH kW inH inW inC stride padding : Nat}
  ...
  let gradOutput' := tensorCast _
    (Pooling.Internal.pool2d_multi_out_shape_eq inC inH inW kH kW stride padding).symm grad_output
