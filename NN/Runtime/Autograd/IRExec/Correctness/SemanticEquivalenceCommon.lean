...
* `permutePackedTensor_eq_applySwapsTensor` (+ helper): connects IR permutation semantics to the runtime
...
When a new proof starts to repeat parent lookup, `Spec.PackedTensor` casting, or tail-of-graph preservation
...
theorem denoteAllState_nil {α : Type} [Context α]
    ...
    denoteAllState (α := α) inShape (st := (⟨[], .nil⟩ : State α inShape)) x =
      #[Spec.PackedTensor.ofTensor x] := by
  simp [denoteAllState, execOfState, ForwardGraph.denoteAll, ForwardGraph.eval,
    packedTensorsOfContext, Spec.PackedTensor.ofTensor, ForwardData.eval,
    Proofs.Autograd.Algebra.TList.toPackedArray, Proofs.Autograd.Algebra.TList.toPackedList]
...
This is used to connect `NN.IR.Graph.permutePackedTensor` to the runtime implementation used by the
...
theorem applySwapsTensor_eq_foldl_swapAdjacentAtDepth
    ...
    (swaps.foldl (fun acc d => Spec.PackedTensor.swapAdjacentAtDepth acc d)
      (Spec.PackedTensor.mk (α := α) s t)) =
      Spec.PackedTensor.mk (α := α) (swapShapeBySwaps s swaps)
  ...
  | cons d ds ih =>
      ...
      change
        (ds.foldl (fun acc d => Spec.PackedTensor.swapAdjacentAtDepth acc d)
            (Spec.PackedTensor.mk (α := α) (s.swapAdjacentAtDepth d)
              (Tensor.swapAdjacentAxes (tensor := t) d))) =
          Spec.PackedTensor.mk (α := α) (swapShapeBySwaps (s.swapAdjacentAtDepth d) ds)
...
/--
Specialize `NN.IR.Graph.permutePackedTensor` to the swap-based implementation used by this lowering pass.

Given a successful `permute?` witness and computed swap depths, `permutePackedTensor` returns the tensor
produced by `applySwapsTensor` with the expected final shape.
-/
theorem permutePackedTensor_eq_applySwapsTensor
    ...
    NN.IR.Graph.permutePackedTensor (α := α) (v := Spec.PackedTensor.mk (α := α) sIn t) perm =
      .ok (Spec.PackedTensor.mk (α := α) (swapShapeBySwaps sIn swaps)
        (applySwapsTensor (α := α) (s := sIn) (swaps := swaps) t)) := by
  -- Unfold `permutePackedTensor`, then rewrite by the permutation witness and swap computation.
  unfold NN.IR.Graph.permutePackedTensor
  simp [hPerm, hSwaps]
...
-- `permutePackedTensor_eq_applySwapsTensor` carries extra witness parameters (`expected`, `swaps`) that are
...
theorem buildFrom_denoteAllFrom_nodeData_exact
    ...
    (hTail :
      NN.IR.Graph.denoteAllFrom (α := α) (g := g) (payload := payload)
          (input := Spec.PackedTensor.mk (α := α) inShape x) (i := i + 1)
    ...
    NN.IR.Graph.denoteAllFrom (α := α) (g := g) (payload := payload)
        (input := Spec.PackedTensor.mk (α := α) inShape x) (i := i)
        (vals := denoteAllState (α := α) inShape (st := (⟨ss, gd⟩ : State α inShape)) x) =
  ...
  have hStep :
      denoteAllState (α := α) inShape
          (st := (⟨ss ++ [τ], .snoc (ss := ss) gd nodeData⟩ : State α inShape)) x =
        vals0.push (Spec.PackedTensor.mk (α := α) τ (nodeData.eval ctx)) := by
