...
* `permuteSomeTensor_eq_applySwapsTensor` (+ helper): connects IR permutation semantics to the runtime
...
When a new proof starts to repeat parent lookup, `Spec.SomeTensor` casting, or tail-of-graph preservation
...
/--
Specialize `NN.IR.Graph.permuteSomeTensor` to the swap-based implementation used by this lowering pass.

Given a successful `permute?` witness and computed swap depths, `permuteSomeTensor` returns the tensor
...
-- `permuteSomeTensor_eq_applySwapsTensor` carries extra witness parameters (`expected`, `swaps`) that are
