...
can dispatch to branch theorems, while this file checks branch-specific lowering pass and evaluator
behavior.

Build note: the random operators are deterministic in the semantics once the seed and node id are
fixed. The proof still has to show that the lowering pass and IR evaluator derive the same key, append a
...
theorem buildFrom_denoteAllFrom_rand_uniform
  ...
  | nil =>
      ...
      have hEval :
        ...
        simp [NN.IR.Graph.evalAt, NN.IR.Graph.evalNode, NN.IR.Graph.normalizeNodeOutput, hN, hk, hp, nodeData, mkForwardNode,
          key, t,
          throw_eq_error]
        rfl
      have hStep :
      ...
      | nil =>
          ...
          | ok ip =>
              ...
              have hRec :
                ...
                exact hBuild
              have hGet :
                  vals0[pId]? = some (Spec.PackedTensor.mk (α := α) Shape.scalar (getIdx (α := α) (xs := ctx) ip)) := by
