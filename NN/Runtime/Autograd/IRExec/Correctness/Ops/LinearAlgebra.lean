...
value produced by the forward-graph node's `forward`.

We handle the two shape cases supported by TorchLean's current matmul lowering pass:
...
- Matmul proofs can be slow because the lowering pass has to distinguish 2D matrix multiplication from
...
/--
Correctness lemma for the `.matmul` lowering step in the 2D matrix-multiply case.
...
/--
Correctness lemma for the `.matmul` lowering step in the batched-matmul (`bmm`) case.
