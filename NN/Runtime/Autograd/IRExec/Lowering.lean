...
The exhaustive checked lowering loop from operation-tagged IR nodes to executable SSA nodes.
-/
...
def buildFrom
  ...
  if h : i < g.nodes.size then
    ...
    let fwd (forward : TList α ([inShape] ++ ss) → Tensor α τ) : ForwardNode α
        ([inShape] ++ ss) τ :=
      mkForwardNode (α := α) (Γ := [inShape] ++ ss) (τ := τ) forward
