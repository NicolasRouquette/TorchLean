...
def generateONNXBridgeScript (opts : ONNXBridgeOptions := {}) : String :=
  joinLines
    ...
    , indentEight "return {\"kind\": \"permute\", \"perm\": perm}"
    ...
    , indentFour "unary = {\"abs\", \"sqrt\", \"inv\", \"relu\", \"tanh\", \"sigmoid\", \"exp\", \"log\", \"sin\", \"cos\", \"softmax\", \"reduce_sum\", \"reduce_mean\", \"reshape\", \"flatten\", \"permute\", \"transpose\"}"
    ...
    , indentEight "raise RuntimeError(f\"Conv: expected OI plus at least one spatial kernel dimension, got {w_shape}\")"
    ...
    , indentEight "raise RuntimeError(f\"Conv: auto_pad={auto_pad!r} is unsupported; export explicit zero-padding extents\")"
    , indentFour "if len(strides) != spatial_rank:"
    , indentEight "raise RuntimeError(f\"Conv: expected {spatial_rank} stride entries, got {strides}\")"
    , indentFour "if len(dilations) != spatial_rank or any(value <= 0 for value in dilations):"
    , indentEight "raise RuntimeError(f\"Conv: expected {spatial_rank} positive dilation entries, got {dilations}\")"
    , indentFour "if len(pads) != 2 * spatial_rank:"
    , indentEight "raise RuntimeError(f\"Conv: expected {2 * spatial_rank} explicit pad entries, got {pads}\")"
    ...
    , indentFour "if len(x_shape) == spatial_rank + 2 and x_shape[0] == 1 and len(out_shape) == spatial_rank + 2 and out_shape[0] == 1:"
    ...
    , indentEight "add_node(\"reshape\", reshape_in, [name_to_id[x_name]], sample_in, {\"in_shape\": x_shape, \"out_shape\": sample_in})"
    , indentEight "add_node(\"conv\", conv_out, [name_to_id[reshape_in]], sample_out, extra)"
    , indentEight "add_node(\"reshape\", out_name, [name_to_id[conv_out]], out_shape, {\"in_shape\": sample_out, \"out_shape\": out_shape})"
    , indentEight "return True"
    , indentFour "raise RuntimeError(f\"Conv: expected channels-first sample or singleton-batched input for spatial rank {spatial_rank}, got {x_shape}\")"
    , ""
    ...
    , indentEight "add_node(\"transpose\", b_t, [b_parent], [b_shape[1], b_shape[0]], {\"axis1\": 0, \"axis2\": 1})"
