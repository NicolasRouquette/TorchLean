...
`Activation.softmaxLastSpec` row-wise to the scaled score matrix.
...
This is purely a property of `softmaxLastSpec`; it does not depend on the particular choice of scores,
or scaling.
-/
theorem scaledDotProductAttention_unmasked_weights_row_sum_one
    ...
    let attentionWeights := Activation.softmaxLastSpec (α := ℝ) scaledScores
    Spec.Tensor.sumSpec (Spec.get attentionWeights i) = 1 := by
  intro scale scores scaledScores attentionWeights
  -- `softmaxLastSpec` is row-wise and the last axis is nonempty because `hK : nK ≠ 0`.
