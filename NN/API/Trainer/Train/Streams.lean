...
def trainStreamFloatWithRun {σ τ : Shape}
  ...
  let evalLoss := do
    Manual.evalMode (task := trainer.task) runner
    TorchLean.Trainer.Manual.moduleLoss (task := trainer.task) runner evalSample
  ...
  let watchEvery := TorchLean.Trainer.Manual.effectiveCudaMemWatch runtimeOpts cfg.steps cudaMemWatch
  let mut memWatch? ← TorchLean.Trainer.Manual.reportCudaMemWatch runtimeOpts watchEvery cfg.steps 0 none
  for step in [0:cfg.steps] do
    let _ ← stepper.stepSample (sampleAt step)
    let done := step + 1
    memWatch? ← TorchLean.Trainer.Manual.reportCudaMemWatch runtimeOpts watchEvery cfg.steps done memWatch?
...
def trainPairStreamFloat {σ₁ τ₁ σ₂ τ₂ : Shape}
  ...
  let watchEvery := TorchLean.Trainer.Manual.effectiveCudaMemWatch runtimeOpts trainOpts.steps cudaMemWatch
  let mut memWatch? ← TorchLean.Trainer.Manual.reportCudaMemWatch runtimeOpts watchEvery trainOpts.steps 0 none
  for step in [0:trainOpts.steps] do
    ...
    for s in secondSamplesAt step do
      ...
      pure ()
    let done := step + 1
    memWatch? ← TorchLean.Trainer.Manual.reportCudaMemWatch runtimeOpts watchEvery trainOpts.steps done memWatch?
    if TorchLean.Training.shouldReport every done then
